...
var (
	...
	dummyCiphertext42_v2 = "v2.QkJCQkJCQkJCQkJCEkUSJcqgA7QWXXC7-gZn_8_lYQI"
	...
	dummyCiphertext55_v2 = "v2.QkJCQkJCQkJCQkJC9X30rkaY8XO5h_ujMLmLXiPzlYA"
)
...
func TestPool_Encrypt(t *testing.T) {
	...
	t.Run("encrypts plaintext successfully with existing key", func(t *testing.T) {
		...
		require.Equal(t, dummyCiphertext55_v2, ciphertext)
	})
	...
	t.Run("encrypts plaintext successfully with new key generation", func(t *testing.T) {
		...
		require.Equal(t, dummyCiphertext42_v2, ciphertext)
	})
...
func TestPool_Decrypt(t *testing.T) {
	...
	versions := []struct {
		...
		{"v1", dummyCiphertext42, dummyCiphertext55},
		{"v2", dummyCiphertext42_v2, dummyCiphertext55_v2},
	}
	...
	for _, v := range versions {
		t.Run(v.label, func(t *testing.T) {
			t.Run("decrypts ciphertext successfully", func(t *testing.T) {
				...
				pool := encryption.NewPool(enc, configs, keysTable, nil)
				plaintext, err := pool.Decrypt(context.Background(), att, "cipherKey4", v.ciphertext55, []byte("aad"))
				require.NoError(t, err)
			...
			t.Run("handles insufficient shares", func(t *testing.T) {
				...
				pool := encryption.NewPool(enc, configs, keysTable, nil)
				plaintext, err := pool.Decrypt(context.Background(), att, "cipherKey4", v.ciphertext42, []byte("aad"))
